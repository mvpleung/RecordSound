...
package com.record.sound.library.view;
...
public class WaveformView_1 extends View {
    ...
    public WaveformView_1(Context context, AttributeSet attrs) {
        ...
        cutPaint.setColor(Color.parseColor("#FF6742"));
        cutPaint.setStrokeWidth(10);
        cutPaint.setAntiAlias(true);

        mSelectedPaint = new Paint();//选择区域
        mSelectedPaint.setAntiAlias(false);
        mSelectedPaint.setColor(Color.parseColor("#EB2E00"));
        mSelectedPaint.setAlpha(60);
...
//    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        centerLine.setColor(Color.parseColor("#999999"));
        ...
        for (int j = 0; j < cutPoint.size(); j++) {
            ...
            if (j < cutPoint.size()) {
                if (touchX > cutPoint.get(j) && touchX < cutPoint.get(j + 1) && touchX != 0) {
                    float[] cutPosition = new float[2];
                    if (selectAreas.containsKey(cutPoint.get(j))) {
                        selectAreas.remove(cutPoint.get(j));
                    } else {
//        					canvas.drawRect(cutPoint.get(j), 0.0F, cutPoint.get(j+1),
//            	        			measuredHeight, mSelectedPaint);
                        cutPosition[0] = cutPoint.get(j);
                        cutPosition[1] = cutPoint.get(j + 1);
                        selectAreas.put(cutPoint.get(j), cutPosition);
                    }
                    touchX = 0;
                    break;
                }
...
//            canvas.drawLine(0, measuredHeight-line_offset/2-1, measuredWidth, measuredHeight-line_offset/2-1, paintLine);//最下面的那根线
        ...
        for (i = 0; i < maxPos(); i++) {
            ...
            if (i + start >= mSelectionStart && i + start < mSelectionEnd) {
                ...
                paint.setColor(Color.parseColor("#FF6742"));
            } else {
        ...
        while (iterator.hasNext()) {
            ...
            canvas.drawRect(fs[0], 0.0F, fs[1], measuredHeight, mSelectedPaint);
        }

        for (int k = 0; k < cutPoint.size(); k++) {
            canvas.drawLine(cutPoint.get(k), 0, cutPoint.get(k), measuredHeight, cutPaint);
        }
     ...
     * @param position
     */
    public void setCutPosition(int position) {
        touchX = 0;
